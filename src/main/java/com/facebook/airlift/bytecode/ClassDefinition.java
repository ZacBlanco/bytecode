...
import static com.facebook.airlift.bytecode.Access.INTERFACE;
...
public class ClassDefinition
{
            ...
            ParameterizedType... interfaceTypes)
    {
        return Joiner.on("").join(
                Stream.concat(ImmutableList.of(classType).stream(), ImmutableList.copyOf(interfaceTypes).stream()).iterator());
    }
    ...
    {
        return Joiner.on("").join(Stream.concat(ImmutableList.of(classType).stream(), Stream.of(interfaceTypes)).iterator());
    }
